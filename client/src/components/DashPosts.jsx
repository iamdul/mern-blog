...
export default function DashPosts() {
  ...
  return (
    <div className='table-auto overflow-x-scroll md:mx-auto scrollbar
    scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500 '>
      {currentUser.isAdmin && userPosts.length>0?(
          ...
          {userPosts.map((post)=>(
            <Table.Body key={post._id} className='divide-y'>
              <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                ...
                <Table.Cell>
                  <span className='foni-medium text-red-500 hover:underline cursor-pointer'>
                    Delete
                  </span>
                </Table.Cell>
                <Table.Cell>
        ...
        {
          showMore && (
            <button onClick={handleShowMore} className='w-full text-teal-500 selt-center text-sm py-7'>
              Show more
            </button>
          )
        }
